...
using LinearAlgebra
using PyPlot
include("withdampingfunctions.jl")
#= STANDARD CONDITIONS
N=2
T=100
Nt=10000
...
#dydt=Nbodydynamics(y(t))
    k1=Nbodydynamics(u,t,kappa,G,ka,kc,d,qlist,mlist)
    k2=Nbodydynamics(u+dt/2*k1,t+dt/2,kappa,G,ka,kc,d,qlist,mlist)
    k3=Nbodydynamics(u+dt/2*k2,t+dt/2,kappa,G,ka,kc,d,qlist,mlist)
    k4=Nbodydynamics(u+dt*k3,t+dt,kappa,G,ka,kc,d,qlist,mlist)
...
for i=1:N
# Define ri, the avg radius of orbit, to be the average distance from the center mass
# ulist is stored as [x1(1),y1(1),x2(1),y2(1),...,xn(1),yn(1),...,x1(Nt),y2,(Nt),...,xn(Nt),yn(Nt)]
# we first need indexes 1, 2N+1, 4N+1,... and 2, 2N+2, 4N+2,...
# all the way up to 2N, 2N+2N, 4N+2N,...
	sumxi=0
	sumyi=0
	...
	rxi=sumxi/Nt # Find avg distance
	ryi=sumyi/Nt
	ri=sqrt(rxi^2+ryi^2)
	println("r$i=$ri")
end