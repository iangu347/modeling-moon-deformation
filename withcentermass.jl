...
using LinearAlgebra
using PyPlot
include("withcentermassfunctions.jl")
#= STANDARD CONDITIONS
N=2
T=100
Nt=10000
...
qlist=.5*[5*10^(-4),5*10^(-4),5*10^(-4),5*10^(-4)]
ka=15
kc=15
# Circular motion
#ui=[-1;0;1;0;0;-0.3;0;0.3]
...
#dydt=Nbodydynamics(y(t))
    k1=Nbodydynamics(u,t,kappa,G,ka,kc,qlist,mlist)
    k2=Nbodydynamics(u+dt/2*k1,t+dt/2,kappa,G,ka,kc,qlist,mlist)
    k3=Nbodydynamics(u+dt/2*k2,t+dt/2,kappa,G,ka,kc,qlist,mlist)
    k4=Nbodydynamics(u+dt*k3,t+dt,kappa,G,ka,kc,qlist,mlist)
