	...
	# we are trying to find overall force that results on ith charge
		...
		for j=1:N
			if i!=j
				...
				mj=mlist[j]
				# electric force
				A=kappa0*q0^2/(G0*M0^2) # coefficient of nondimensionalization
			...
			B=ka0*L0^3/(G0*M0^2) # coefficient of nondimensionalization
		...
		C=kc0*L0^3/(G0*M0^2) # coefficient of nondimensionalization
