...
function Nbodydynamics(u,t,kappa,G,ka,kc,d,qlist,mlist)
	...
	# we are trying to find overall force that results on ith charge
		...
		for j=1:N
			...
			tempaxispring=ka*rmagnext*riinext[1]/mi
			tempayispring=ka*rmagnext*riinext[2]/mi
		...
		ri11mag=norm(ri11)
		ri11=ri11/ri11mag
		tempaxispring=kc*ri11mag*ri11[1]/mi
		tempayispring=kc*ri11mag*ri11[2]/mi
		overallaxi=overallaxi+tempaxispring
		overallayi=overallayi+tempayispring
        # add earths at [-10,0] and [10,0]
        m12=10^11
        m13=10^11
        ri12=[-10-xi;0-yi]
        ri13=[10-xi;0-yi]
		...
		overallayi=overallayi+tempayigravity
		# damping
		overallaxi=overallaxi-d*(u[2*N+(2*i-1)]*ri11[1])*ri11[1]
		overallayi=overallayi-d*(u[2*N+(2*i)]*ri11[2])*ri11[2]
        # set to acceleration
